...
if ($reset == 1) {
	...
	$row = mysql_fetch_assoc($res);
	if (!empty($message)) {
		if ($row['Lastmessage'] == NULL) mysql_query("UPDATE User SET Lastmessage=CONCAT('" . $row['Lastmessage'] . "','" . $message . "') WHERE HWID=" . $hwid . "");
		else mysql_query("UPDATE User SET Lastmessage=CONCAT('" . $row['Lastmessage'] . "','
" . $message . "
') WHERE HWID=" . $hwid . "");
	}
